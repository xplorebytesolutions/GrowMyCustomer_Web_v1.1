        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-6 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
             <div className="relative h-16 w-16 flex items-center justify-center">
                <svg className="h-full w-full -rotate-90 text-slate-100" viewBox="0 0 36 36">
                   <path className="text-slate-100" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="4" />
                   <path className="text-emerald-500 drop-shadow-md transition-all duration-1000 ease-out" 
             ...
             <div>
                <p className="text-sm font-semibold text-slate-900">{completedSteps} of {totalSteps} Steps Complete</p>
                <p className="text-xs text-slate-500 mt-1">Next: {whatsappConnected ? "Choose a Plan" : "Connect WhatsApp"}</p>
             </div>
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-5 p-6 lg:p-8 gap-6 items-center">
                 ...
                 <div className="lg:col-span-3 space-y-4">
                    <div className="inline-flex items-center gap-2 px-2.5 py-0.5 bg-emerald-50 text-emerald-700 rounded-lg text-[10px] font-bold uppercase tracking-wider">
                       <Zap size={12} className="fill-emerald-700" /> Recommended Next Step
                    </div>
                    ...
                    <p className="text-slate-600 text-sm leading-relaxed max-w-xl">
                       This is the official Meta integration. No workarounds, no risks. 
                       Get verified, send campaigns, and manage team inboxes instantly.
                    </p>
                    
                    {/* Reduced gap from 4 to 2, text from sm to xs/sm */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 pt-1">
                       {["Meta Verified Flow", "Use Existing Number", "Official Display Name", "24/7 Reliability"].map((item, i) => (
                    ...
                    <p className="text-[10px] text-slate-400">
                       By connecting, you agree to Meta's Commerce Policy. Setup takes ~2 minutes.
                    </p>
                 </div>
                 ...
                 <div className="lg:col-span-2 relative hidden lg:block h-full min-h-[14rem] flex items-center justify-center">
                    ...
                    <div className="relative z-10 bg-gradient-to-br from-slate-50 to-white p-4 rounded-xl border border-slate-100 shadow-sm transform rotate-2 hover:rotate-0 transition-transform duration-500 max-w-[280px]">
                       ...
                       <div className="absolute bottom-3 left-3 right-3 bg-white/90 backdrop-blur-sm p-3 rounded-lg border border-slate-100 shadow-sm flex items-center gap-2">
                          ...
                          <div>
                             <p className="text-[11px] font-bold text-slate-800">100% Secure Integration</p>
                             <p className="text-[9px] text-slate-500">End-to-end encrypted by Meta</p>
                          </div>
                       </div>
                    </div>
                 </div>
      ...
      <AnimatePresence>
        ...
        {showApplyModal && (
          <ApplyModal 
             ...
             onMigrate={() => { setShowApplyModal(false); setShowMigration(true); }}
...
function StepCard({ step, title, desc, icon: Icon, status, statusTone, isActive, to, onClick }) {
      ...
      >
         ...
         <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3">
               <div className={`flex h-10 w-10 items-center justify-center rounded-full text-xs font-bold border ${
                  isActive ? "bg-emerald-100 text-emerald-700 border-emerald-200" : "bg-slate-50 text-slate-500 border-slate-100"
               }`}>
                  {step}
               </div>
               <div>
                  <h3 className="text-sm font-bold text-slate-900 flex items-center gap-1.5">
                     {title}
                     {statusTone === "success" && <CheckCircle2 size={14} className="text-emerald-500" />}
                  </h3>
                  <p className="text-xs text-slate-500 leading-snug max-w-[12rem]">{desc}</p>
               </div>
            </div>
            {Icon && (
               <div className={`p-2 rounded-lg ${isActive ? "bg-emerald-50 text-emerald-600" : "bg-slate-50 text-slate-400"}`}>
                  <Icon size={18} />
               </div>
            )}
         </div>
         ...
         <div className="mt-4 flex items-center justify-between gap-2 border-t border-slate-50 pt-3">
            <span className={`inline-flex items-center rounded-md border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${toneClasses}`}>
               {status}
            </span>
               ...
               {!submitted ? (
                  <>
                     <div className="flex items-center justify-between mb-6">
                        <h2 className="text-2xl font-bold text-slate-900">Migrate to XploreByte</h2>
                        <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full text-slate-500 transition-colors">✕</button>
                     </div>
                     ...
                     <div className="space-y-4 mb-8">
                        ...
                        <p className="text-slate-600 text-sm">
                           Our migration team will handle the technical transfer. Once you submit, we'll reach out within 24 hours to guide you through the number porting process.
                        </p>
                     </div>

                     <div className="flex gap-3 justify-end">
                        <button onClick={onClose} className="px-5 py-2.5 text-slate-600 font-semibold hover:bg-slate-50 rounded-lg transition-colors">Cancel</button>
                        <button onClick={onSubmit} className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-lg shadow-sm transition-colors">
                           Request Migration
                        </button>
                     </div>
                  </>
                  ...
                  <div className="text-center py-8">
                     ...
                     <button onClick={() => { setSubmitted(false); onClose(); }} className="px-8 py-2.5 bg-slate-900 text-white font-semibold rounded-lg hover:bg-slate-800 transition-colors">
                        Done
                     </button>
                  </div>
               )}
      ...
      <div className="fixed inset-0 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm z-[100] p-4">
         ...
         >
            <div className="p-6 md:p-8 overflow-y-auto">
               <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold text-emerald-950">Connect WhatsApp API</h2>
                  <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full text-slate-500 transition-colors">✕</button>
               </div>

               <div className="space-y-6">
                  <div className="bg-emerald-50 rounded-xl p-5 border border-emerald-100">
                     <p className="text-xs font-bold text-emerald-800 uppercase tracking-wide mb-3">Before you start, keep this ready</p>
                     <ul className="space-y-2">
                        ...
                        <li className="flex items-start gap-2.5 text-sm text-emerald-900">
                           <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-emerald-500 shrink-0" />
                           Access to the phone number you want to use (to receive OTP via SMS or call)
                        </li>
                        <li className="flex items-start gap-2.5 text-sm text-emerald-900">
                           <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-emerald-500 shrink-0" />
                           A Facebook account with permission to manage your business (or ability to create one)
                        </li>
                     </ul>
                  </div>
                  ...
                  <div className="border border-emerald-100 rounded-xl overflow-hidden bg-white">
                     ...
                     >
                        <span className="font-semibold text-sm">What happens after you click "Continue With Facebook"?</span>
                        <ChevronRight size={18} className={`text-emerald-600 transition-transform duration-200 ${showDetails ? 'rotate-90' : ''}`} />
                     </button>
                           ...
                           >
                              <div className="p-5 border-t border-emerald-100 text-[13px] text-slate-700 leading-relaxed">
                                 <ol className="list-decimal list-outside ml-4 space-y-2">
                                    <li>A secure Meta (Facebook) window will open in a new tab or popup.</li>
                                    <li>Log in with the Facebook account that manages your business (or create one if needed).</li>
                                    <li>Select or create your <span className="font-semibold text-slate-900">Business Manager</span> and <span className="font-semibold text-slate-900">WhatsApp Business Account</span>.</li>
                                    <li>Choose the phone number you want to use (new number or move an existing WhatsApp Business / WhatsApp API number).</li>
                                    <li>Verify the number using the SMS or call OTP that Meta sends to that phone.</li>
                                    <li>Review and grant the requested permissions so XploreByte can send and receive WhatsApp messages on your behalf.</li>
                                    <li>When Meta shows "<span className="font-semibold text-slate-900">Setup complete</span>", close the Facebook window and return to this dashboard.</li>
                                    <li>Your WhatsApp connection status in XploreByte will update automatically once Meta confirms the setup.</li>
                                 </ol>
                              </div>
                           </motion.div>
                        )}
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row gap-4 justify-center items-center">
               ...
               >
                  {connecting && <Loader2 className="h-4 w-4 animate-spin text-white" />}
