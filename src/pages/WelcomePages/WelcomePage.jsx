...
const VerticalStepper = ({ whatsappConnected, hasPlan, onConnect, onNavigate }) => {
  return (
    <div className="space-y-1">
      <VerticalStep
        ...
        status="Created"
        isCompleted={true}
        onClick={() => onNavigate("/app/settings/profile-completion")}
      />
      <VerticalStep
        step={2}
      ...
      <VerticalStep
        ...
        isLast={true}
        onClick={() => !hasPlan && onNavigate("/app/settings/billing")}
      />
    </div>
  );
};
...
export default function WelcomePage() {
  ...
  useEffect(() => {
    ...
    if (status === "success") {
      ...
      sessionStorage.removeItem("xb_pending_esu_pin");
      setShowPinModal(false);
      fetchWaStatus(); // Refresh status to show connected state
    } 
